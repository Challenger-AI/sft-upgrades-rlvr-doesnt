...
tokenizer = AutoTokenizer.from_pretrained("Qwen/Qwen2.5-32B-Instruct") # base model does not have a chat template
...
qwq_acc_txt = f"base_{passk} accuracy: {acc}\n"
print(qwq_acc_txt)

import os

if not os.path.exists("./src/base/results.txt"):
    open("src/base/results.txt", "w").close()

with open("src/base/results.txt", "a") as f:
    f.write(qwq_acc_txt)