...
tokenizer = AutoTokenizer.from_pretrained(model_name) # base model does not have a chat template
...
qwq_acc_txt = f"ds_sft_{passk} accuracy: {acc}\n"
print(qwq_acc_txt)

import os

if not os.path.exists("./src/deepseek/results.txt"):
    open("src/deepseek/results.txt", "w").close()

with open("src/deepseek/results.txt", "a") as f:
    f.write(qwq_acc_txt)